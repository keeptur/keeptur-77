...
period_comparison AS (
  ...
  SELECT 
    ...
    SUM(monthly_revenue_cents) FILTER (WHERE updated_at >= CURRENT_DATE - INTERVAL '30 days') as current_revenue,
    SUM(monthly_revenue_cents) FILTER (WHERE updated_at >= CURRENT_DATE - INTERVAL '60 days' 
                                       AND updated_at < CURRENT_DATE - INTERVAL '30 days') as previous_revenue
  FROM payment_data
)
SELECT 
  -- Total metrics
  ...
  CASE 
    WHEN pc.previous_subs > 0 THEN 
      ROUND(((pc.current_subs::float - pc.previous_subs::float) / pc.previous_subs::float) * 100, 1)
    ELSE 0 
  END as subscription_growth_percentage,
  
  CASE 
    WHEN pc.previous_revenue > 0 THEN 
      ROUND(((pc.current_revenue::float - pc.previous_revenue::float) / pc.previous_revenue::float) * 100, 1)
    ELSE 0 

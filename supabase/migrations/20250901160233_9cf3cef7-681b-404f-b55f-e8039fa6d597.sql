...
BEGIN
  ...
  IF relkind IN ('r','p') THEN
    ...
    IF NOT EXISTS (
      ...
      EXECUTE 'CREATE POLICY "admin_metrics_admin_select" ON public.admin_metrics FOR SELECT USING (has_role(auth.uid(), ''admin''::app_role))';
    END IF;

    -- Lock down mutations by NOT creating INSERT/UPDATE/DELETE policies

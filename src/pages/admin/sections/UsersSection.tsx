      ...
      <Card className="rounded-xl p-0 border bg-card">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading && <tr><td className="py-6 px-2 text-sm text-muted-foreground" colSpan={8}>Carregando...</td></tr>}
              {!loading && filtered.length === 0 && <tr><td className="py-6 px-2 text-sm text-muted-foreground" colSpan={8}>Nenhum usuário encontrado.</td></tr>}
              ...
              return <tr key={u.email} className="border-b last:border-b-0">
                    ...
                    <td className="py-3 px-2">
                      <span className="px-2 py-1 rounded-full text-xs font-medium" style={plan === 'Pro' ? badgeStyle('plan-pro') : plan === 'Enterprise' ? badgeStyle('plan-enterprise') : badgeStyle('plan-basic')}>{plan}</span>
                    </td>
                    <td className="py-3 px-2">
                      <span className="px-2 py-1 rounded-full text-xs font-medium" style={status === 'active' ? badgeStyle('status-active') : status === 'trial' ? badgeStyle('status-trial') : badgeStyle('status-inactive')}>{status === 'active' ? 'Ativo' : status === 'trial' ? 'Trial' : 'Inativo'}</span>
                    </td>
                    <td className="py-3 px-2">
                      <input type="number" placeholder="+" className="w-16 px-2 py-1 text-xs border rounded text-center" onKeyDown={e => {
                    if (e.key === 'Enter') {
                    ...
                    <td className="py-3 px-2">
                      <div className="flex gap-1">
                        ...
                        {u.id && isAdmin(u.id) && <Button size="sm" variant="destructive" className="h-6 px-2 text-xs" onClick={() => demote(u.id!)}>
                            Remover
                          </Button>}
                      </div>
                    </td>

...
export function PersonModal({ person, isOpen, mode, onClose, onSave }: PersonModalProps) {
  ...
  useEffect(() => {
    if (person) {
      ...
      // Verificar se é para criar pessoa jurídica
      const isCompany = person?.attributes?.kind === "company";
      setFormData({
        name: "",
        "company-name": "",
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="kind">Tipo de Pessoa</Label>
                {isEditing ? (
                  <Select
                    value={formData.kind}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {formData.kind === "individual" ? (
                  <>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="passport-number">Número do Passaporte</Label>
                      {isEditing ? (
                        <Input
                          id="passport-number"
                        ...
                        <p className="text-sm py-2">{formData["passport-number"] || "Não informado"}</p>
                      )}
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="passport-expiration">Validade do Passaporte</Label>
                      {isEditing ? (
                        <Input
                          id="passport-expiration"
                          type="date"
                          value={formData["passport-expiration"]}
                          onChange={(e) => handleInputChange("passport-expiration", e.target.value)}
                        />
                      ) : (
                        <p className="text-sm py-2">
                          {formData["passport-expiration"] ? format(new Date(formData["passport-expiration"]), "dd/MM/yyyy", { locale: ptBR }) : "Não informado"}
                        </p>
                      )}
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="city-inscription">Inscrição Municipal</Label>
                      {isEditing ? (
                        <Input
                          id="city-inscription"
                          value={formData["city-inscription"]}
                          onChange={(e) => handleInputChange("city-inscription", e.target.value)}
                        />
                      ) : (
                        <p className="text-sm py-2">{formData["city-inscription"] || "Não informado"}</p>
                      )}
                    </div>
                  </>
                )}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm py-2 min-h-[100px]">{formData.observations || "Nenhuma observação"}</p>
                )}
              </div>
            </div>
            ...
            {person && (
              <div className="pt-4 border-t text-xs text-muted-foreground">
                <p>
                  Cadastrado em {format(new Date(person.attributes["registered-at"]), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                </p>
              </div>
            )}
          </div>

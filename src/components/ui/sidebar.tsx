...
/* ==== Helpers para evitar React.Children.only ==== */
function ensureSingleChild(children: React.ReactNode) {
  if (React.Children.count(children) <= 1) return children ?? null;
  return <span className="contents">{children}</span>;
}
function WithSafeSlot({
  asChild,
  fallbackTag,
  children,
  ...rest
}: { asChild?: boolean; fallbackTag: any; children?: React.ReactNode } & React.ComponentProps<any>) {
  if (asChild) return <Slot {...rest}>{ensureSingleChild(children)}</Slot>;
  const Comp = fallbackTag;
  ...
  return (
    ...
    >
      ...
      >
        <div
          data-sidebar="sidebar"
          className="flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow bg-sidebar"
        >
          {children}
        </div>
      </div>
